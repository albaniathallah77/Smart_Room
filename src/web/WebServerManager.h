...
<script>
  ...
  function handleState(event) {
    ...
    tv.textContent = s.tv ? (s.happyMode ? 'HAPPY' : (s.kenzieMode ? 'KENZIE' : (s.kacauMode ? 'KACAU' : (s.stikmanMode ? 'STIKMAN' : (s.catMode ? 'CAT' : (s.fightMode ? 'FIGHT' : 'ON')))))) : 'OFF';
